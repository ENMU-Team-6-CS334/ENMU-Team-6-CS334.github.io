...
window.onload = getFlavorsAPI(),closeForm(),closeCreateForm();
...
function addFlavor(flavor, price, img){
  ...
  let postJSON = JSON.stringify(flavorPost)
  console.log(postJSON)
  fetch("http://127.0.0.1:5000/flavors/", {
    method: "POST",
    body: JSON.stringify(flavorPost),
    headers: {
...
function removeFlavor(flavorEntered){
  ...
  // k = 0;
  let examplePost = {
    flavor: flavorEntered,
    price: 0,
    image: "image"
  };
  let postJSON = JSON.stringify(examplePost)
  console.log(postJSON)
  fetch("http://127.0.0.1:5000/flavors/", {
    method: "DELETE",
...
function editFlavor(flavorEntered, newFlavor, newPrice, newImg){
  removeFlavor(flavorEntered)
  addFlavor(newFlavor,newPrice,newImg)
  getFlavorsAPI();
  updateCards();
  location.reload(true)
}
...
function setEditExample(enteredFlavor){
  ...
  let selectedEditPrice = selectedExampleFlavor.price;
  let selectedEditFlavor = selectedExampleFlavor.flavor;
...
function userAddFlavor(){
  ...
  addFlavor(userAddFlavor,userAddPrice,'image');
  closeCreateForm();
